...
const ExportButton = ({ data, fileName, fields }) => {
    const handleExport = async () => {
        if (!data || data.length === 0) {
            // PopUp.failed('No data to import');
...
const ExportExcel = (arrData, fileName, fields) => {
    const handleExport = async () => {
        if (!data || data.length === 0) {
            alert('No data to export!');
            return;
        ...
        try {
            ...
            console.error('Export error ', error);
            alert('Error exportng data: Please try again')
        }
    };

    return (
        <button onClick={handleExport}
        className="border-1 rounded-l mt-3 px-2 py-0,5 text-green-600 hover:text-white hover:bg-green-600 active:bg-green-800"
        >
            Export to Excel
        </button>
    )
